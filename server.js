...
const WORD_BANK = {
  career: {
    easy: ["周报","月报","年报","加班","KPI","OKR","团建","改需求","打卡","汇报","提案","实习","面试","升职","加薪","离职","入职","报销","请假","出差","培训","早会","例会","站会","周会","月会","季度会","总结会","启动会","评审会","庆功宴","散伙饭","头脑风暴","一对一","视频会","PPT","文档","表格","邮件","群聊","语音","钉钉","飞书","企微","腾讯会议","屏幕共享","云文档","知识库","键盘","鼠标","显示器","工牌","工位","会议室","白板","投影仪","打印机","咖啡机","饮水机","老板","同事","甲方","乙方","客户","需求","猎头","简历","合同","工资","年终奖","奖金","五险一金","公积金","社保","个税","绿植","升降桌","防窥膜","降噪耳机","下午茶","零食","发票","年假","报税","共享文档","在线表格","需求评审","Bug","测试","改bug","准点下班","咖啡杯","订书机","线上会议","需求变更","版本迭代","早八","日报","晨会","撤回键","表情包","需求池","排期表","居家办公","弹性打卡"],
    hard: ["摸鱼","画饼","甩锅","背锅","划水","救火","冲刺","跳槽","裁员","复盘","排期","绩效","发版","画大饼","背锅侠","摸鱼怪","划水王","画饼侠","甩锅侠","摸鱼学","准点跑路","假装忙","已读不回","消息红点","键盘侠","打工魂","带薪摸鱼","摸鱼搭子","工位摸鱼","带薪健身","工位养生","在线摸鱼","摸鱼神器"]
  },
  meme: {
    easy: ["雪王","蜜雪冰城","流量","热搜","顶流","网红","联名","周边","二创","鬼畜","名场面","高能","萌宠","手工","露营","飞盘","雪豹","遥遥领先","已读乱回","啊对对对","上分","电子榨菜","多巴胺","酱香拿铁","茅台","演唱会","音乐节","美拉德","暴风吸入","C位","本命","出道","成团","电子木鱼","路人粉","爆料","实锤","打工人","干饭人","尾款人","工具人","社畜"],
  ...
  life: {
    easy: ["奶茶","火锅","猫咪","可乐","雨伞","咖啡","啤酒","炸鸡","烧烤","串串","麻辣烫","螺蛳粉","煎饼","包子","饺子","面条","寿司","披萨","汉堡","薯条","蛋糕","面包","甜甜圈","冰淇淋","水果","苹果","香蕉","西瓜","草莓","葡萄","橙子","芒果","榴莲","桃子","樱桃","小狗","兔子","仓鼠","乌龟","金鱼","鹦鹉","多肉","鲜花","玫瑰","向日葵","书本","小说","漫画","电影","电视剧","综艺","游戏","钢琴","吉他","跑步","健身","瑜伽","游泳","篮球","足球","羽毛球","骑行","钓鱼","旅行","拍照","自拍","美甲","化妆","香水","口红","面膜","睡衣","枕头","被子","大床","沙发","抱枕","台灯","蜡烛","香薰","露营车","骑行服","钓鱼佬","猫罐头","狗狗","仓鼠球","盲盒","手办","汉服","洛丽塔","奶茶杯","瑞幸","蜜雪","麻辣香锅","烤冷面","煎饼果子","钵仔糕","麻薯","司康","可颂","巴斯克","猫窝","狗窝","猫爬架","逗猫棒","桨板","滑雪","冲浪","攀岩","密室逃脱","剧本杀","桌游","麻将","掼蛋","围炉煮茶","露营帐篷","天幕","野餐","滑板","陆冲","猫咖","狗咖","撸猫","撸狗","手作","陶艺","烘焙","咖啡拉花","精酿","微醺","露营椅"],
    hard: []
  }
};
...
function pickWord() {
  ...
  // 70% 抽简单、30% 抽困难（该类无困难词时回退简单）
  const useHard = Math.random() < 0.3 && bank.hard.length > 0;
  const arr = useHard ? bank.hard : (bank.easy.length ? bank.easy : bank.hard);
